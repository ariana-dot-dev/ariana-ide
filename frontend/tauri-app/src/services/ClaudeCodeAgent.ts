import { TerminalSpec, TerminalEvent, LineItem, CustomTerminalAPI } from "./CustomTerminalAPI";
...
export class ClaudeCodeAgent extends CustomTerminalAPI {
	...
	constructor() {
		...
		console.log(this.logPrefix, "resizeTerminal method:", this.resizeTerminal.toString());
	}
	...
	async startTask(
		...
		try {
			...
			console.log(this.logPrefix, "Terminal connected with ID:", this.terminalId);
	...
	private async initializeClaudeCode(): Promise<void> {
		...
		this.emit('taskStarted', { prompt: this.currentPrompt, terminalId: this.terminalId });
	}
	...
	private async sendSingleKey(key: KeyboardKey): Promise<void> {
		...
		switch (key.type) {
			...
			case 'ctrl':
				...
				keyData = String.fromCharCode(key.value.toUpperCase().charCodeAt(0) - 64);
				break;
			case 'special':
				switch (key.value) {
					case 'Enter': keyData = '\r'; break;
					case 'Backspace': keyData = '\b'; break;
	...
	private checkForTaskCompletion(): void {
		...
		try {
			const recentLines = this.screenLines.slice(-5).map(line => 
				line.map(item => item.lexeme).join('')
			);
	...
	private async processTuiInteraction(tuiLines: TuiLine[]): Promise<void> {
		...
		const hasEnterToConfirm = newLines.some(line => line.includes("Enter to confirm"));
		const hasTrustQuestion = newLines.some(line => line.includes("Do you trust the files in this folder?"));
		
		if (hasEnterToConfirm && hasTrustQuestion) {
			console.log(this.logPrefix, "Found trust confirmation prompt, sending Enter...");
			await this.sendRawInput(this.terminalId, "\r");
			return;
		}

		// Check for "Yes, and don't ask again this session (shift+tab)"
		const hasShiftTabOption = newLines.some(line => line.includes("Yes, and don't ask again this session (shift+tab)"));
		if (hasShiftTabOption) {
			console.log(this.logPrefix, "Found 'don't ask again' option, sending Shift+Tab...");
		...
		if (hasTryPrompt && !this.hasSeenTryPrompt && this.currentPrompt) {
			console.log(this.logPrefix, "Found 'Try' prompt, sending task prompt:", this.currentPrompt);
		...
		const hasPromptWithoutTry = newLines.some(line => 
			line.includes("│ > ") && !line.includes("│ > Try")
		);
		if (hasPromptWithoutTry && this.hasSeenTryPrompt) {
			console.log(this.logPrefix, "Task appears to be complete, sending Ctrl+D twice...");
			await this.sendCtrlD(this.terminalId);
			await this.delay(1000);
			await this.sendCtrlD(this.terminalId);
			return;
		...
		const hasEscToInterrupt = newLines.some(line => line.includes("esc to interrupt"));
