...
export interface CursorCommand {
  type: 'CUU' | 'CUD' | 'CUF' | 'CUB' | 'CNL' | 'CPL' | 'CHA' | 'CUP' | 'ED' | 'EL' | 'SU' | 'SD';
  params: number[];
}
...
export class CustomTerminalAPI {
  ...
  async connectTerminal(spec: TerminalSpec): Promise<string> {
    try {
      const terminalId = await invoke<string>('custom_connect_terminal', { spec });
      return terminalId;
    } catch (error) {
   ...
   */
  async onTerminalEvent(id: string, callback: (event: TerminalEvent) => void): Promise<void> {
    try {
  ...
  async onTerminalDisconnect(id: string, callback: () => void): Promise<void> {
    try {
      ...
      throw new Error(`Failed to set up terminal disconnect listener: ${error}`);
    }
  }
