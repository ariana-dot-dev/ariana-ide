 ...
 */
export function resolveColor(name: string, cssVars: CSSStyleDeclaration = getComputedStyle(document.documentElement)): string {
  const cssVal = cssVars.getPropertyValue(name).trim();
  return cssVal || (COLOR_VARS as Record<string, string>)[name] || FALLBACK_COLOR;
}
