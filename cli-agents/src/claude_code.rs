use crate::{
	ClaudeCodeTaskResult, CliAgentError, FileWatcher, KeyboardKey, TaskResult, TuiLine,
};
...
impl ClaudeCodeTask {
	pub async fn wait_till_finish(&self) -> ClaudeCodeTaskResult {
		self.wait_till_finish_with_timeout(Duration::from_secs(300))
			.await
			.unwrap_or(ClaudeCodeTaskResult::Error("Timeout".to_string()))
	}

	pub async fn wait_till_finish_with_timeout(
		&self,
		timeout_duration: Duration,
	) -> Result<ClaudeCodeTaskResult, CliAgentError> {
	...
	async fn wait_internal(&self) -> ClaudeCodeTaskResult {
		...
		if inner.completed {
			return inner
				.result
				.clone()
				.unwrap_or(ClaudeCodeTaskResult::Error("No result".to_string()));
		}
		...
		let diff = match inner.file_watcher.compute_diff() {
			Ok(d) => d,
			Err(e) => {
				return ClaudeCodeTaskResult::Error(format!(
	...
	pub fn build(self) -> Result<ExternallyManagedClaudeCode, CliAgentError> {
		Ok(ExternallyManagedClaudeCode {
			path: self.path.ok_or_else(|| {
				CliAgentError::TaskExecutionError("Path is required".to_string())
			})?,
			tui_receiver: self.tui_receiver.ok_or_else(|| {
				CliAgentError::TaskExecutionError("TUI receiver is required".to_string())
			})?,
			keyboard_sender: self.keyboard_sender.ok_or_else(|| {
				CliAgentError::TaskExecutionError(
					"Keyboard sender is required".to_string(),
				)
			})?,
		})
	}
...
impl ClaudeCodeInterface for ExternallyManagedClaudeCode {
	async fn prompt(&mut self, message: &str) -> ClaudeCodeTask {
		...
		let message_keys: Vec<KeyboardKey> = message
			.chars()
	...
	async fn install() -> Result<(), CliAgentError> {
		...
		Err(CliAgentError::TaskExecutionError(
			"Installation not implemented".to_string(),
		))
	}
